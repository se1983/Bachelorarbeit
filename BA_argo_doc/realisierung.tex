...
\\
Damit werden die ArgoFloats auf einer Vektoriellen Karte angezeigt. Die Karte nimmt auch bereits Steuerungsgesten entgegen. Was zu diesem Zeitpunkt noch fehlt, ist die Möglichkeit mit den ArgoFloats zu interagieren. Bei Hovern mit des Zeigers über einer ArgoFloat soll ein Tooltip mit einigen Informationen erscheinen und die Messstation optisch hervorgehoben werden.  Die Hervorhebung wird durch den Befehl \jsinline{map.addInteraction(hoverInteraction);} in der Karte registriert. \texttt{overInteraction} ist eine Instanz von \jsinline{ol.interaction.Select} und erlaubt die Auswahl von Features und des Event-Typs. In diesem Fall werden alle Features des argoFloatsLayer beim überfahren mit der Maus ausgewählt. Openlayers wählt mit dieser Konfiguration einen Vergrößerungseffekt beim überfahren eines ArgoFloatFeatures. 

Um den Tooltip zur Anzeige zu bekommen, wurde eine weitere Funktion definiert. In Listing \ref{lst:pointermove} ist die Funktion für das Abfangen des \texttt{pointermove}-Events zu sehen.
...
Auch ier wird die \texttt{map.on}-Funktion der Karte verwendet um den Event zu extrahieren. Ist der Event vom Typ \jsinline{'click'} so werden die darauf folgenden Funktionen mit dem Pixel an der Position des Mauszeigers aufgerufen.

Die Funktion \jsinline{displayArgoData} ist für die Darstellung der Messdaten und Informationen über die Sidebar verantwortlich. Für die Anzeige der Messwerte wird der div-Container mit der ID \texttt{\#chart-picture} ausgewählt. 
 
\jsinline{displayPositions}
